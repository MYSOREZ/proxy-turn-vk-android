...
func main() {
	...
	// Форвард расшифрованного трафика на произвольный локальный UDP-порт
	// вместо встроенного WireGuard. Путь данных в handleConn — обычный UDP-
	// релей (net.Dial на wgEndpoint), протокол внутри он не разбирает,
	// поэтому туда одинаково ложится и WireGuard, и QUIC от Hysteria2.
	// Встроенный WG продолжает слушать свой -wg-port и просто не
	...
	flag.Parse()
	if s := strings.TrimSpace(*wgIface); s != "" {
		wgIfaceName = s
	}
	dns = *dnsFlag
	...
	// Режим проверки пароля: ничего не поднимаем и ничего не пишем в базу.
	if *authCheck != "" {
		os.Exit(runAuthCheck(*configDir, mainPasswordValue, *authCheck))
	}
	...
	//
	// В режиме -forward он не поднимается вовсе. Во-первых, raw-путь имеет
	// собственный TUN/NAT и уходит в сеть мимо -forward — то есть мимо
	// Hysteria2, ради которой этот режим и существует. Во-вторых, имя
	// интерфейса, подсеть, метка iptables и таблица nft у него захардкожены:
	// второй экземпляр сервера на той же машине снёс бы `wdttraw0` соседа и
	// перехватил его NAT-правила. Раньше это ещё и роняло сервер на старте с
	// кодом 1 (bind на чужой UDP-порт: "address already in use").
	if forwardMode && *listenRaw != "" {
		log.Printf("[RAW] Листенер %s не поднимается: активен режим -forward", *listenRaw)
	} else if *listenRaw != "" {
