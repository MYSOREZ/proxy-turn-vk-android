...
func main() {
	...
	connMode := flag.String("mode", "vpn", "режим клиента (vpn|socks|rawtun|hysteria)")
	hy2Pass := flag.String("hy2-pass", "", "пароль Hysteria2 (только -mode hysteria)")
	hy2SNI := flag.String("hy2-sni", "bing.com", "TLS SNI для Hysteria2")
	...
	if activeConnMode != "socks" && activeConnMode != "rawtun" && activeConnMode != "hysteria" {
		activeConnMode = "vpn"
	}
	if activeConnMode == "hysteria" && *hy2Pass == "" {
		log.Fatal("[HY2] Для -mode hysteria нужен -hy2-pass")
	}
	if (activeConnMode == "socks" || activeConnMode == "hysteria") && *socksAuth {
	...
	// Режим Hysteria2: TURN-воркеры ниже уже слушают localPort и гонят всё
	// на сервер, поэтому клиенту Hysteria2 достаточно бить QUIC в этот
	// локальный порт. Туннель поднимается не мгновенно, а QUIC-хендшейк
	// ждать не умеет — поэтому переподключаемся в цикле, пока не встанет.
	if activeConnMode == "hysteria" {
		hyParams := HysteriaParams{
			ServerAddr: *listen,
			Auth:       *hy2Pass,
			SNI:        *hy2SNI,
