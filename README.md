...
## AI-адаптивная маскировка трафика (`-ai-listen` / `-ai-obfs`, опционально)

Помимо статичной RTP-обфускации (`ObfsConfig`/`ObfsState`, ChaCha20-Poly1305)
добавлен **опциональный, полностью независимый** слой `aiobfs`: трафик
постоянно меняет вид (профиль аудио/видео/демо-экрана/keepalive, тайминги,
паддинг — автокоррелированным блужданием, а не случайно-независимо на
каждый пакет) на основе адверсариального бандита (EXP3 с вычитанием
базовой линии) и маленькой онлайн-обучаемой нейросети (5→8→N), реагируя на
измеренные RTT/потери — а не большой LLM в пути пакета (это было бы слишком
медленно). Подробный разбор архитектуры, гарантий безопасности и того, что
именно (не) решает эта схема против ML-классификаторов — в исходном
проекте: [core/README.md](https://github.com/MYSOREZ/vk-turn-proxy-android/blob/claude/proxy-ai-traffic-masking-p5qx6q/core/README.md).

**Важно:** wire-формат `aiobfs` (AES-256-GCM, свой nonce из RTP-заголовка)
**не совместим побайтово** со старой обфускацией (ChaCha20-Poly1305, другой
порядок полей в nonce). Поэтому это отдельный, парный флаг с обеих сторон,
а не замена — старый `-listen`/обычные клиенты работают ровно как раньше.

**Сервер** (`server.go`):
```bash
./wdtt-server -listen 0.0.0.0:56000 -ai-listen 0.0.0.0:56002 -password ...
```
`-ai-listen` — дополнительный DTLS-адрес; можно не указывать (по умолчанию
выключено). Использует те же пароли (`serverWrapKeys`), тот же WireGuard
бэкенд — просто другой слой обфускации на другом порту.

**Клиент** (`go_client`):
```bash
./client -peer <IP_сервера>:56002 -ai-obfs -password ... -vk ...
```
`-peer` должен указывать на `-ai-listen`-порт сервера, а не на обычный
`-listen`. `-ai-obfs` без совпадающего `-ai-listen` на сервере просто не
установит соединение (разные протоколы).

Реализация (`aiobfs/` в корне — серверный модуль, `go_client/aiobfs/` —
клиентский; код продублирован между двумя независимыми Go-модулями по той
же причине, что и существующий `obfs.go`/`server`-эквивалент) — портирована
из [core/aiobfs](https://github.com/MYSOREZ/vk-turn-proxy-android/tree/claude/proxy-ai-traffic-masking-p5qx6q/core/aiobfs)
проекта `vk-turn-proxy-android`, вместе с тестами.
...
go_client/     Нативный клиент → libclient.so
  aiobfs/      Опциональный слой ИИ-маскировки трафика (см. выше)
server.go      Серверная часть (совместим с WDTT)
server_ai.go   Опциональный -ai-listen слушатель (см. выше)
aiobfs/        Тот же слой ИИ-маскировки для серверного модуля
