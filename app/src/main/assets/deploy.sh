...
# Терпимость к простою. По умолчанию QUIC рвёт сессию после 30 секунд тишины,
# а телефон в дозе замолкает именно так — и клиент каждый раз пересоздавал
# сессию после короткого сна. Значение должно совпадать с клиентским
# (go_client/hysteria.go), иначе сервер разорвёт первым.
quic:
  maxIdleTimeout: 90s
